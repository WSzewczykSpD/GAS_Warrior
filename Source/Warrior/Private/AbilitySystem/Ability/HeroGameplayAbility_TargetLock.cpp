...
void UHeroGameplayAbility_TargetLock::OnTargetLockTick(float DeltaTime)
{
	...
	if(bShouldOverrideRotation)
	{
		...
		GetHeroCharacterFromActorInfo()->SetActorRotation(FRotator(0.f, TargetRot.Yaw,0.0f)); 
	}
...
void UHeroGameplayAbility_TargetLock::SwitchTarget(const FGameplayTag& InSwitchDirectionTag)
{
	...
	{
		NewTargetToLock = GetNearestTargetFromAvaliableActors(ActorsOnLeft);
	}else
	{
		NewTargetToLock = GetNearestTargetFromAvaliableActors(ActorsOnRight);
	}
	...
	if(TargetLockWidgetSize == FVector2D::ZeroVector)
	{
		DrawnTargetLockWidget->WidgetTree->ForEachWidget(
			[this](UWidget* FoundWidget)
			{
				...
				{
					TargetLockWidgetSize.Set(FoundSizeBox->GetWidthOverride(),FoundSizeBox->GetHeightOverride());
				}
			}
		);
